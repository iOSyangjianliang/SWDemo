...
@property (nonatomic, assign) NSUInteger jl_signatureCacheIndex;
@end

@implementation MBProgressHUD (JLToastDisplayPloy)
-(void)setJl_signatureCacheIndex:(NSUInteger)jl_signatureCacheIndex
{
    objc_setAssociatedObject(self, @"jl_signatureCacheIndex", @(jl_signatureCacheIndex), OBJC_ASSOCIATION_ASSIGN);

}
-(NSUInteger)jl_signatureCacheIndex
{
    return [objc_getAssociatedObject(self, @"jl_signatureCacheIndex") boolValue];
}
...
+ (void)jl_showMessage:(nullable NSString *)message customIcon:(nullable NSString *)imageName view:(nullable UIView *)view hideAfterDelay:(NSTimeInterval)delay displayPloy:(MBProgressHUDDisplayPloy)ploy isQueue:(BOOL)queue
{
    ...
    if (jl_isNeedDisplayPloy) {
        hud.jl_signatureCacheIndex = signatureCache.count-1;
    }
...
//            NSLog(@"%@",hud);
            ...
            //自定义toast策略目前再移除toast碰到问题、暂时还未解决
            if (signatureCache.count>hud.jl_signatureCacheIndex) {
                NSLog(@"正在删除%d",hud.jl_signatureCacheIndex);
                [signatureCache removeObjectAtIndex:hud.jl_signatureCacheIndex];
            }
            if (signatureCache.count>0)
            {//eg:已有三个使用默认策略toast正在不同view展示，若此时第四个等待策略toast，那么第四个toast将等待正在展示的三个toast均隐藏后再展示（此次还没实现）
               //对于有正在等待toast、此时展示默认策略toast的处理？？考虑到方案需要考虑情况太过于复杂、暂时停止自定义策略，等有时间来
...
//                    NSLog(@"222222");
                ...
                NSLog(@"当前队列%lu个",(unsigned long)signatureCache.count);
...

{
    ...
    { //如果是等待展示的不再加入
        ...
        {//如果有正在展示的Toast
            ...
            else if (ploy == MBProgressHUDDisplayWaitingIfViewSame)
            {
                [MBProgressHUD cacheAllParametersWithMessage:message customIcon:imageName view:view hideAfterDelay:delay displayPloy:ploy];
